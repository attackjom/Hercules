...
#define set_sc_with_vfx(skill, sc, flag) do { status->set_sc((skill), (sc), (flag)); status->dbs->IconChangeTable[sc].relevant_bl_types |= BL_SCEFFECT; } while(0)
	...
	status->set_sc( PR_IMPOSITIO         , SC_IMPOSITIO       , SCB_WATK
#ifdef RENEWAL
		|SCB_MATK );
#else
		);
	...
	status->set_sc( BS_ADRENALINE        , SC_ADRENALINE      , SCB_ASPD|SCB_HIT );
	...
	status->set_sc( SA_FLAMELAUNCHER     , SC_PROPERTYFIRE    , SCB_ALL );
	status->set_sc( SA_FROSTWEAPON       , SC_PROPERTYWATER   , SCB_ALL );
	status->set_sc( SA_LIGHTNINGLOADER   , SC_PROPERTYWIND    , SCB_ALL );
	status->set_sc( SA_SEISMICWEAPON     , SC_PROPERTYGROUND  , SCB_ALL );
	status->set_sc( SA_VOLCANO           , SC_VOLCANO         , SCB_WATK|SCB_MATK );
	...
	status->set_sc( BD_ADAPTATION		 , SC_ADAPTATION      , SCB_NONE );
	status->set_sc( BD_RICHMANKIM        , SC_RICHMANKIM      , SCB_NONE );
	...
	status->set_sc( BD_ROKISWEIL         , SC_ROKISWEIL		  , SCB_NONE );
	status->set_sc( BD_INTOABYSS         , SC_INTOABYSS  	  , SCB_NONE );
	...
	status->set_sc( BA_POEMBRAGI         , SC_POEMBRAGI		  , SCB_NONE );
	...
	status->set_sc( DC_FORTUNEKISS       , SC_FORTUNE         , SCB_ALL );
	...
	status->set_sc( HP_BASILICA          , SC_BASILICA        , SCB_ALL  );
	...
	status->set_sc( CG_SPECIALSINGER     , SC_LONGING          , SCB_SPEED|SCB_ASPD );
	status->set_sc( CG_HERMODE           , SC_HERMODE         , SCB_NONE );
